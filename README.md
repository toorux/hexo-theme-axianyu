...
一个极简风格的Hexo主题, 基于[adoubi][2]主题, 修改了排版样式, 兼容手机端处理, 修改TOC为HEXO自带TOC, 请勿安装 `hexo-toc`; 并且`确保Hexo版本 > 3.0`
...
> 关于上图
> 1. 单词Hexo左右使用margin替代空格（无论你是否有手动输入空格），在单词和中文之间添加间距增强可读性。
> 2. 标点符号句号和括号之间有明显重叠，即为中文标点间距挤压的表现（中文符号中，句号之后与左括号之前都会有较长空白间距）
...
在文章的 `Front-matter` 部分增加 `toc: true` 即可显示目录. 更多关于 `Front-matter` 参考 [Hexo doc: Front-matter][3]
...
主题不具有评论功能，但右上角的Email链接可以自动唤起用户的邮件程序（如果有的话），并自动填充模版， 效果如下：
<img src="/images/mailto.png" style="max-height: 70vh">
...
[Wikipedia: 知识共享许可协议][1]  
[hexo-theme-adoubi][2]  
[Hexo Doc: Front-matter][3]  
[Hexo Doc: Configuration][4]  
[~~Fonts for Apple Platforms~~][5]   
[normalize.css][6]   
[赫蹏（hètí）][7]

[1]: https://zh.wikipedia.org/wiki/知识共享许可协议
[2]: https://github.com/shinux/hexo-theme-adoubi
